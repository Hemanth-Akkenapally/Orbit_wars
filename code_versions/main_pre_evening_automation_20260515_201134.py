...
def _agent_impl(obs, config=None):
    ...
    try:
        ...
        if group_actions:
            return sanitize_actions(group_actions, state)
        ...
        return sanitize_actions(actions, state)
    except Exception:
...
def bowwow_neutral_chain(source, budget, state):
    ...
    for target in state["planets"]:
        ...
        if distance > 40:
            margin += 1
        ships = int(max(MIN_SEND, math.ceil(target["ships"] + margin)))
        ...
        if not safe_launch_path(source, angle, distance, state):
            continue
        eta = travel_turns(distance, ships)
        value = target["production"] * 34.0 - ships * 1.20 - distance * 0.28 - eta * 0.85
        if target["production"] >= 4:
            value += 18.0
        if target["ships"] <= 12:
            value += 8.0
